...
const (
	...
	defaultSchedulingThroughputInterval = 1 * time.Second // 5 * time.Second
)
	...
	go func() {
		...
		for {
			...
			case <-time.After(measurmentInterval):
				...
				klog.Infof("%v: %s: %d pods scheduled, throughput: %f", s, selector.String(), lastScheduledCount, throughput)
			}
		}
	}()
	return nil
