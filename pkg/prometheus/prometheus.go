...
func InitFlags(p *config.PrometheusConfig) {
	...
	flags.BoolEnvVar(&p.TearDownServer, "tear-down-prometheus-server", "TEAR_DOWN_PROMETHEUS_SERVER", false, "Whether to tear-down the prometheus server after tests (if set-up).")
